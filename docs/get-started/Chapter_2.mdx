...
**_token_**-ы. То есть, буквально, токен `while` (цикл _while_ в JavaScript) побуквенно или,
согласно _Lexical grammar_, по каждому **_code point_** был собран в **_input element_** `while`,
который также является **_token_**-ом в рамках уже _Syntactic grammar_, отвечающей за синтаксис.
