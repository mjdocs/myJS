---
id: environment-record
slug: /records/environment-record
title: Environvent Record или Funarg проблема
description: Environment Record chapter
sidebar_position: 0
sidebar_label: 4.1. Environvent Record или Funarg проблема
---

import ChapterInfo from '@site/src/components/ChapterInfo/ChapterInfo'
import DocCardList from '@theme/DocCardList'

<ChapterInfo type='formal' complexity='normal' time='?' />

# Глава 4.1: Environvent Record или Funarg проблема

:::warning

Глава находится в разработке!

:::

В данной главе хочется раскрыть известную в программировании Funarg проблему, при чём тут
_Environment Record_, и показать, как в языке ECMAScript, согласно спецификации, происходит её
решение. Сразу стоит оговориться, что общие термины, используемые при её решении, и алгоритм могут
не соответствовать тому, как это описано в спецификации языка ECMAScript. Но для понимания причины
появления проблемы и её всестороннего исследования стоит начать с теории.
